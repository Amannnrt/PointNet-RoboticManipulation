...
# -----------------------------
# Object Configuration (change these to switch between cube and sphere)
OBJECT_TYPE = "sphere"  # Options: "cube" or "sphere"
SPHERE_RADIUS = 0.025
CUBE_HALF_EXTENTS = [0.025, 0.025, 0.025]
OBJECT_POSITION = [0.5, 0.0, 0.025]  # Same for both (adjust z for sphere if needed)

# Pick-up Preferences (set True to pick up, False to skip)
PICK_UP_CUBE = True
PICK_UP_SPHERE = True
...
CLASS_NAMES = ["Cube", "Sphere"]  # Model outputs these classes
NUM_POINTS = 1024
...
def setup_simulation():
    ...
    # Create object based on OBJECT_TYPE
    if OBJECT_TYPE.lower() == "sphere":
        shape = p.createCollisionShape(p.GEOM_SPHERE, radius=SPHERE_RADIUS)
        visual = p.createVisualShape(p.GEOM_SPHERE, radius=SPHERE_RADIUS, rgbaColor=[0, 0, 1, 1])  # Blue
        obj_pos = [OBJECT_POSITION[0], OBJECT_POSITION[1], OBJECT_POSITION[2]]
    elif OBJECT_TYPE.lower() == "cube":
        shape = p.createCollisionShape(p.GEOM_BOX, halfExtents=CUBE_HALF_EXTENTS)
        visual = p.createVisualShape(p.GEOM_BOX, halfExtents=CUBE_HALF_EXTENTS, rgbaColor=[0, 1, 0, 1])  # Green
        obj_pos = OBJECT_POSITION
    else:
        raise ValueError("OBJECT_TYPE must be 'cube' or 'sphere'")
    ...
    return robot_id, object_body, tray_id, plane_id
...
def move_robot(robot_id, object_id, tray_id, predicted_class):
    # Determine object type and pick-up decision
    is_sphere = OBJECT_TYPE.lower() == "sphere"
    predicted_sphere = predicted_class == 1  # "Sphere" class
    object_name = "sphere" if is_sphere else "cube"
    predicted_name = "sphere" if predicted_sphere else "other"

    # Check pick-up preferences
    should_pick = False
    if is_sphere and PICK_UP_SPHERE and predicted_sphere:
        should_pick = True
    elif not is_sphere and PICK_UP_CUBE and not predicted_sphere:
        should_pick = True

    print(f"\nObject is a {object_name}, predicted as {predicted_name}")
    if not should_pick:
        print(f"Skipping manipulation based on preferences (PICK_UP_CUBE={PICK_UP_CUBE}, PICK_UP_SPHERE={PICK_UP_SPHERE})")
        return
    print("Starting manipulation sequence...")

    # Define heights
    grasp_offset = SPHERE_RADIUS if is_sphere else CUBE_HALF_EXTENTS[2]
    grasp_height = OBJECT_POSITION[2] + grasp_offset + 0.01
    approach_height = grasp_height + 0.05
    transit_height = TRAY_POSITION[2] + 0.15  # Clear tray walls (0.1m)
    tray_height = TRAY_POSITION[2] + 0.05
    tray_approach_height = tray_height + 0.05

    # Waypoints
    targets = [
        [OBJECT_POSITION[0], OBJECT_POSITION[1], approach_height],
        [OBJECT_POSITION[0], OBJECT_POSITION[1], grasp_height],
        [OBJECT_POSITION[0], OBJECT_POSITION[1], transit_height],
        [(OBJECT_POSITION[0] + TRAY_POSITION[0]) / 2, (OBJECT_POSITION[1] + TRAY_POSITION[1]) / 2, transit_height],
        [TRAY_POSITION[0], TRAY_POSITION[1], tray_approach_height],
        [TRAY_POSITION[0], TRAY_POSITION[1], tray_height],
        [TRAY_POSITION[0], TRAY_POSITION[1], transit_height],
    ]
        ...
        if dist < 0.002:
            ...
            if move_stage == 1 and not attached:
                ...
                constraint_id = p.createConstraint(
                    ...
                    p.JOINT_FIXED,
                    [0, 0, 0],
                    [0, 0, -grasp_offset],  # Align with object center
                    [0, 0, 0]
                )
                attached = True
...
if __name__ == "__main__":
    print(f"Starting simulation with {OBJECT_TYPE}...")
    robot_id, object_id, tray_id, plane_id = setup_simulation()
    ...
    point_cloud = capture_point_cloud(object_id)
    ...
    move_robot(robot_id, object_id, tray_id, predicted_class)
