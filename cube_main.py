...
def setup_simulation():
    ...
    # Create cube
    cube_shape = p.createCollisionShape(p.GEOM_BOX, halfExtents=CUBE_HALF_EXTENTS)
    cube_visual = p.createVisualShape(p.GEOM_BOX, halfExtents=CUBE_HALF_EXTENTS, rgbaColor=[0, 1, 0, 1])
    ...
    return robot_id, cube_body, tray_id, plane_id
...
def capture_point_cloud(object_id):
    view_matrix = p.computeViewMatrixFromYawPitchRoll(
        cameraTargetPosition=CUBE_POSITION,
...
def move_robot(robot_id, object_id, tray_id, predicted_class):
    ...
    grasp_height = CUBE_POSITION[2] + CUBE_HALF_EXTENTS[2] + 0.01
    approach_height = grasp_height + 0.05
    transit_height = TRAY_POSITION[2] + 0.15  # Clear tray walls (0.1m high)
    ...
    targets = [
        [CUBE_POSITION[0], CUBE_POSITION[1], approach_height],  # Approach above cube
        [CUBE_POSITION[0], CUBE_POSITION[1], grasp_height],     # Grasp position
        [CUBE_POSITION[0], CUBE_POSITION[1], transit_height],   # Lift high after grasping
        # Intermediate point to avoid walls
        [(CUBE_POSITION[0] + TRAY_POSITION[0]) / 2, (CUBE_POSITION[1] + TRAY_POSITION[1]) / 2, transit_height],
    ...
    gripper_closed = [0.01, 0.01]  # Gripper closed for cube
        ...
        if dist < 0.002:
            ...
            if move_stage == 1 and not attached:
                ...
                constraint_id = p.createConstraint(
                    ...
                    p.JOINT_FIXED,
                    [0, 0, 0],
                    [0, 0, -CUBE_HALF_EXTENTS[2]],  # Offset to align with cube center
                    [0, 0, 0]
                )
                attached = True
...
if __name__ == "__main__":
    robot_id, cube_id, tray_id, plane_id = setup_simulation()
    ...
    point_cloud = capture_point_cloud(cube_id)
    ...
    move_robot(robot_id, cube_id, tray_id, predicted_class)
